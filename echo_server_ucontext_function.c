...
static ssize_t
async_send(async_context_t *context, int fd, void *buffer, size_t len, int flags)
{
	struct io_uring_sqe *sqe = io_uring_get_sqe(context->uring);
	io_uring_prep_send(sqe, fd, buffer, len, 0 /* flags */);
...
static ssize_t
async_recv(async_context_t *context, int fd, void *buffer, size_t len, int flags)
{
	struct io_uring_sqe *sqe = io_uring_get_sqe(context->uring);
	io_uring_prep_recv(sqe, fd, buffer, len, 0 /* flags */);
	io_uring_sqe_set_data(sqe, context);
...
static int
async_close(async_context_t *context, int fd)
{
	struct io_uring_sqe *sqe = io_uring_get_sqe(context->uring);
...
worker(async_context_t *context, int fd)
{
	for (;;) {
		...
		ssize_t io_res = async_recv(context, fd, buffer, sizeof(buffer), 0 /* flags */);
		if (io_res <= 0) {
			break;
		}
		len += io_res;
		...
		while (len > 0) {
			ssize_t io_res = async_send(context, fd, &buffer[offset], len, 0);
